...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fetchChart = async () => {
      ...
      try {
        const res = await axios.get(`${API_BASE}/chart/${selectedTicker}?period=${chartPeriod}`)
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-4 font-mono text-xs border-t border-border pt-4 mt-4">
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <span className="text-primary">{stock.Conviction || 'N/A'}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  <table className="w-full text-left font-mono text-xs">
                    <thead>
                      <tr className="border-b border-border text-sub uppercase tracking-widest bg-black/50">
                        ...
                        <th className="p-4 font-normal text-right">Fwd P/E</th>
                        <th className="p-4 font-normal text-right">Debt/Eq</th>
                    ...
                    <tbody>
                      {data.map((row, i) => (
                        ...
                        >
                          ...
                          <td className="p-4 text-right text-primary">{row.Conviction || 'N/A'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="flex flex-col xl:flex-row gap-8">
                ...
                <div className="w-full xl:w-1/4 flex flex-col gap-6">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-4 bg-card border border-border px-4 py-3 hover:border-primary/50 transition-colors">
                  ...
                  <div className="border border-border bg-card p-6">
                    <h3 className="font-mono text-xs uppercase tracking-widest text-primary mb-4 border-b border-border pb-2">Technical Snapshot</h3>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <div className="font-mono text-[10px] text-muted uppercase">Tech Score</div>
                        <div className={`font-mono text-sm mt-1 ${colorCode(selectedAsset?.Tech_Score)}`}>{num(selectedAsset?.Tech_Score)}</div>
                      </div>
                      <div>
                        <div className="font-mono text-[10px] text-muted uppercase">Conviction</div>
                        <div className="font-mono text-sm mt-1 text-primary">{selectedAsset?.Conviction || 'N/A'}</div>
                      </div>
                      <div>
                  ...
                  <div className="border border-border bg-card p-6">
                    <h3 className="font-mono text-xs uppercase tracking-widest text-primary mb-4 border-b border-border pb-2">Fundamentals</h3>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <div className="font-mono text-[10px] text-muted uppercase">Fund Score</div>
                        <div className={`font-mono text-sm mt-1 ${Number(selectedAsset?.Fund_Score) >= 5 ? 'text-green-500' : 'text-primary'}`}>{num(selectedAsset?.Fund_Score)}</div>
                      </div>
                      <div>
                ...
                <div className="w-full xl:w-3/4 flex flex-col gap-6">
                  ...
                  {/* Period Toggles */}
                  <div className="flex bg-card border border-border p-1 rounded-sm w-fit">
                    {['3mo', '6mo', '1y', '2y', '5y'].map(p => (
                      <button 
                        key={p}
                        onClick={() => setChartPeriod(p)}
                        className={`px-4 py-2 font-mono text-xs uppercase tracking-widest transition-all ${chartPeriod === p ? 'bg-brand text-white shadow-sm' : 'text-muted hover:text-primary'}`}
                      >
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="border border-border bg-card p-6 h-[450px] flex flex-col relative">
                        ...
                        <div style={{ width: '100%', height: '100%', minHeight: '350px' }}>
                          <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={chartData}>
                              ...
                              <Bar yAxisId="volume" name="Volume" dataKey="volume" fill="#27272A" barSize={4} />
                  ...
                  <div className="border border-border bg-card mt-4">
                    <div className="p-6 border-b border-border">
                      <h3 className="font-display text-xl tracking-wide text-primary">Sector Peer Comparison</h3>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left font-mono text-xs">
                        <thead>
                          <tr className="border-b border-border text-sub uppercase tracking-widest bg-black/50">
                            <th className="p-4 font-normal">Ticker</th>
                            <th className="p-4 font-normal text-right">Market Cap</th>
                            <th className="p-4 font-normal text-right">Tech Score</th>
                            <th className="p-4 font-normal text-right">Fund Score</th>
                            <th className="p-4 font-normal text-right">P/E</th>
                            <th className="p-4 font-normal text-right">Debt/Eq</th>
