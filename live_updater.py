...
def update_live_prices():
    ...
    if "data" not in market_data or not isinstance(market_data["data"], list):
        logger.error("Invalid JSON structure: 'data' key missing or malformed.")
        return
    ...
    for row in market_data["data"]:
        ...
        try:
            ...
            if len(closes) >= 2:
                ...
                if abs(chg) > 50:
                    logger.warning(f"Suspicious price move for {ticker}: {chg:.2f}%. Skipping.")
                    continue
    ...
    _archive_live_prices(market_data, nifty_data, now_time)


def _archive_live_prices(market_data: dict, nifty_data: dict, now_time: datetime):
    """Archive live prices to SQLite with deduplication."""
    try:
        import sqlite3
        ...
        ''')
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_lp_ticker ON live_prices(Ticker)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_lp_time ON live_prices(Timestamp)")
        ...
        for row in market_data.get("data", []):
            if "Price" in row and "1d_Chg_%" in row and "Ticker" in row:
                price = row["Price"]
                chg = row["1d_Chg_%"]
                if price is not None and chg is not None and price > 0:
                    records_to_insert.append((row["Ticker"], timestamp_str, price, chg))
                    
        if nifty_data and nifty_data.get("price"):
            records_to_insert.append(("^NSEI", timestamp_str, nifty_data["price"], nifty_data["change_pct"]))
            
        cursor.executemany(
            "INSERT OR REPLACE INTO live_prices (Ticker, Timestamp, Price, Change_Pct) VALUES (?, ?, ?, ?)",
            records_to_insert
        )
        
        conn.commit()
        conn.close()
        logger.info(f"Archived {len(records_to_insert)} live prices to database.")
    except Exception as e:
        logger.error(f"Failed to archive live prices: {e}")
